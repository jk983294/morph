...
GAME_ROWS = 6
GAME_COLS = 7
BITS_IN_LEN = 3  # specifies how many bits are used to encode the height of the column
PLAYER_BLACK = 1
PLAYER_WHITE = 0
COUNT_TO_WIN = 4
...
def _check_won(field, col, delta_row):
    ...
    :param delta_row:
        0, checks for horizontal won,
        1 for rising diagonal
        -1 for falling diagonal
    """
    player = field[col][-1]
...
def move(state_int, col, player):
    ...
    :return: tuple of (state_new, won). won is bool
    """
    assert isinstance(state_int, int)
    ...
    won_ = suff == [player] * COUNT_TO_WIN
    if not won_:
        won_ = _check_won(field, col, 0) or _check_won(field, col, 1) or _check_won(field, col, -1)
    return encode_lists(field), won_
...
if __name__ == '__main__':
    state_ = encode_lists([[]] * GAME_COLS)
    print(state_)
    print(decode_binary(state_))
    p_moves = possible_moves(state_)
    print(p_moves)
    state_new, won = move(state_, p_moves[0], PLAYER_WHITE)
    print(state_new, won)
    pic = render(state_new)
    print(pic)
